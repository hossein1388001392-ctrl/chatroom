...
function show(m){let d=document.createElement("div");d.className="bubble "+(m.username===user.username?"mine":"");d.dataset.id=m.id;let time=new Date(m.time||Date.now()).toLocaleTimeString("fa-IR",{hour:"2-digit",minute:"2-digit"});let x=`<small>${m.username}</small><br>${esc(m.text)}<div class="time">${time}</div>`;if(m.file?.type?.startsWith("image/"))x+=`<br><img src="${m.file.url}">`;else if(m.file?.type?.startsWith("video/"))x+=`<br><video src="${m.file.url}" controls></video>`;else if(m.file)x+=`<br><audio src="${m.file.url}" controls></audio>`;d.innerHTML=x+`<button class="del" onclick="delMsg("${m.id}")">🗑</button>`;$("messages").appendChild(d);$("messages").scrollTop=$("messages").scrollHeight}
